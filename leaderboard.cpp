...
void arrangetic(int n)
{
	...
	for (int pass = 0; pass < n; pass++)
	{
		...
		{
			pos = my.tellg();
			my.read((char*)&tic1, sizeof(tic1));
			my.read((char*)&tic2, sizeof(tic2));
			if (tic1.wins < tic2.wins)
			{
				my.seekg(pos);
				my.write((char*)&tic2, sizeof(tic2));
				my.write((char*)&tic1, sizeof(tic1));
			}
...
int check;
char choice;
//left-1,up-2,down-3,right-4,stop-0
int dir;
...
void Snake(char ch_1[])
{
	...
	snac temp;
	system("cls");
	cout << "enter level of hardness (10-100)(smaller the number faster the movement speed)(recommended beginner level- 80): ";
	cin >> h;
	setup();
	while (!gameover)
	{
	...
	if (m == 'y' || m == 'Y')
	{
		ofstream fout("Leaderboard5.dat", ios::binary | ios::app);
		strcpy_s(snac1.name, ch_1);
		snac1.score = score;
		fout.write((char*)&snac1, sizeof(snac1));
...
void TicTacToeSingle(char call[])
{
	...
	else
	{
		...
		while (f.read((char*)&tic1, sizeof(tic1)))
		{
			...
			{
				tic1.wins = tic1.wins + 1;
				f.seekg(pos * sizeof(tic1));
				f.write((char*)&tic1, sizeof(tic1));
			}
		}
		f.close();
	}
...
int main()
{
	...
	do
	{
		...
		cout << "				Main Menu				";
		char choice_1;
		cout << endl << "a.Snake" << endl << "b.Tic Tac Toe(single player)" << endl << "c.Tic Tac Toe(double player)" << endl << "d.Snake Leaderboard " << endl << "e.Tic Tac Toe Leaderboard" << endl;
		cin >> choice_1;
		switch (choice_1)
		{
		case 'a':
